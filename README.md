...
Static Gallery is a CLI application. Run `uv gallery --help` from your
...
`--config`: sets the root configuration file (dafult is `site.conf` in the
...
**Language**: the locale and langauge code of the site (e.g. "en-us")
...
separater, and is discarded, and the fourth line is passed on to the markdown
...
3. Static assets (`*.css`, `*.js`, and anything else) will mapped directly
...
contents of `about.md` and `about.jpg` is simple treated as a static asset.
