...
- `nodes.py` — `Node` class with typed child collections (`pages`, `images`, `assets`, `dirs`). Node types: HOME, DIRECTORY, GALLERY, MARKDOWN, IMAGE, STATIC.
...
- `builder.py` — `Builder` class that walks the node tree, renders each node through Jinja2 templates, writes HTML to `public_path`, and copies images/assets. Template selection: `default.html` for MARKDOWN/HOME/DIRECTORY-with-index, `directory.html` for DIRECTORY-without-index, `gallery.html` for GALLERY. Files in the theme's `static/` subdirectory are copied to the public root.
