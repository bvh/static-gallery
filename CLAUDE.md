...
- `node.py` — `Node` class with typed child collections (`pages`, `images`, `assets`, `dirs`). Node types: HOME, DIRECTORY, GALLERY, PAGE, IMAGE, STATIC. IMAGE nodes have a lazy `metadata` property that reads EXIF/IPTC/XMP data on first access and an optional `content_path` for paired markdown. `index_path` stores the path to an `index.md` file for container nodes. `template_name` property returns the appropriate template (`image.html`, `gallery.html`, `directory.html`, or `default.html`). `title_fallback` property returns `stem` for PAGE and IMAGE nodes, `name` for others. `build_image_data(img)` is a module-level helper that builds a dict from an IMAGE node's metadata, name, and stem — used by both `builder.py` and `shortcodes.py`.
