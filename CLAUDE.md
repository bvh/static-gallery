...
**Data flow:** CLI args → `StaticGalleryConfig` → `Scanner(config).scan(path)` builds a recursive `StaticGalleryNode` tree → `Renderer(config).render(root, path)` walks the tree, renders Jinja2 templates, writes HTML and copies assets to `public_path`.

**Key modules:**
- `cli.py` — argparse-based CLI with optional flags (`--title`, `--language`, `--url`, `--config`, `--theme`, `--public`). Builds a `StaticGalleryConfig`, scans source, and renders via `Renderer`.
