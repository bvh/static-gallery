...
**Data flow:** CLI args → `Config` → `Scanner(config).scan(path)` returns a `Site` (root `Node` tree + `SuffixIndex`) → `Builder(config).render(site)` walks the tree, renders Jinja2 templates, writes HTML and copies assets to `public_path`.
...
- `scanner.py` — `Scanner` class that recursively walks directories, classifies files by extension, creates node tree. `scan()` returns a `Site` object containing the root node, source path, and an optional `SuffixIndex`. Uses a two-phase scan: first collects all entries, then pairs markdown files with images by stem (`photo.md` + `photo.jpg` → paired). Paired markdown sets `content_path` on the IMAGE node instead of creating a PAGE node, preserving gallery classification. Index files (`index.md`, `index.html`, `index.htm`) become parent container text (not a separate node). Non-index HTML/HTM files are classified as PAGE nodes (not paired with images). `site.conf` at the root is loaded into config (not a separate node). Directories with an index file become PAGE type (page bundles). Directories with only images (and no index, statics, subdirs, or unpaired pages) become GALLERY type; other mixed content becomes DIRECTORY.
- `site.py` — `Site` class that bundles the root `Node` tree, `source_path`, and `SuffixIndex` into a single object. The `index` property lazily builds a `SuffixIndex` from the tree if one wasn't provided at construction. Returned by `Scanner.scan()` and consumed by `Builder.render()`.
- `node.py` — `Node` class with typed child collections (`pages`, `images`, `assets`, `dirs`). Node types: HOME, DIRECTORY, GALLERY, PAGE, IMAGE, STATIC. PAGE nodes include both single pages (files) and page bundles (directories with index files). IMAGE nodes have a lazy `metadata` property that reads EXIF/IPTC/XMP data on first access and an optional `content_path` for paired markdown. `index_path` stores the path to an index file for container nodes and page bundles. Classification methods: `is_image()` (.jpg, .jpeg, .webp, .png, .gif), `is_markdown()` (.md, .markdown), `is_html()` (.html, .htm). `is_gallery()` returns true only when a directory contains exclusively images (no pages, statics, dirs, or index file). `template_name` property returns the appropriate template: HOME→`home.html`, PAGE→`page.html`, IMAGE→`image.html`, GALLERY→`gallery.html`, DIRECTORY→`directory.html`. `title_fallback` property returns `stem` for PAGE and IMAGE nodes, `name` for others. `build_image_data(img)` is a module-level helper that builds a dict from an IMAGE node's metadata, name, and stem — used by both `builder.py` and `shortcodes.py`.
